...
def find_closest_centroids(X, centroids):
    ...
    for i in range(m):
        ...
        for j in range(K):
            ...
            distances[j] = np.sum(diff ** 2)  # squared L2 distance
...
def compute_centroids(X, idx, K):
    ...
    ### START CODE HERE ###
    for k in range(K):
        points = X[idx == k]      # all points assigned to centroid k
...
```
    ...
    <matplotlib.image.AxesImage at 0x7ef4a181e290>
...
```
    ...
    Shape of idx: (16384,)
    Closest centroid for the first five elements: [ 0 11 11  0  0]
